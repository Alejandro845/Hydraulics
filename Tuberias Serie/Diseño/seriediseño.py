...
tub1=[-0.0451, 0, 0, 0.85, 65000.0, 350, 1.5e-06, 7.9]
tub2=[-0.039, 0, 0, 0, 0, 123, 1.5e-06, 3.3]
tub3=[-0.0732, 0, 0, 0, 0, 174, 1.5e-06, 3.5]
...
while True:
    ...
    for tub in sist.values():
        ...
        for c in range(len(diametros)):
            ...
            #While para calcular el valor de f con punto fijo 
            while True:
        ...
        VR=Q/(((math.pi)/4)*(D**2)) #Calculo de la velocidad real
        ...
        p=p+1 #variable contadora para saber en que tubería estoy

    hmvf=HT-sum(iterahf)-sum(iterahe)
    
    print(f"El balance de energía al final de la iteración fue: {hmvf:.6f}")

    if abs(hmvi-hmvf)<=error or hmvf<0 or j>10: #Convergencia GLOBAL de las iteraciones
        break
    
    hmvi = hmvf #Actualizo la energía

    u = iterahf #almaceno en dos listas aparte los valores de cada iteración
    v = iterahe

    iterahf=[] #limpio las listas con las que se calculó hmvf para la siguiente iteración
    iterahe=[]

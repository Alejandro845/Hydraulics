...
########## CALCULO DE COMPROBACIÓN DE DISEÑO PARA TUBERÍAS EN SERIE - EJEMPLO 1 ##########
...
tub1=[-0.06, 0, 0, 0.6096, 423, 0.0003, 4.2]
tub2=[-0.074, 0, 0, 0.508, 174, 1.5e-06, 3.4]
tub3=[-0.060, 0, 0, 0.3048, 373, 1.5e-06, 5.3]
...
while True:
    ...
    for tub in sist.values():
        ...
        if tub == sist['tub4']: #valor maximo de tuberias
            HG+=ko*((V**2)/(2*g))+he+hf
        if tub == sist['tub1']:
            HG+=ki*((V**2)/(2*g))+he+hf
        if tub == sist['tub2'] or tub == sist['tub3']:
            HG+=he+hf
